...
export default function RecentActivity({
  ...
  return (
    <section tabIndex={0} className="rounded-2xl border border-gray-200 bg-surface-container-lowest p-6 shadow-sm transition-colors duration-200 ease-in-out hover:border-primary focus:outline-none focus:border-primary">
      ...
      <div className="space-y-4">
        {items.map((item) => (
          <div key={`${item.title}-${item.time}`} tabIndex={0} className="flex gap-3 rounded-xl bg-surface p-3 border border-gray-200 transition-colors duration-200 ease-in-out hover:border-primary focus:outline-none focus:border-primary focus:shadow-sm">
            <div
